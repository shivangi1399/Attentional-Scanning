...
phase_folder = '/mnt/hpc/projects/MWSampling/4Shivangi/results_klecks/phase_coherence/ph55_till_100';
...
CriticalTime_mat(~isnan(CriticalTime_mat)) = min(CriticalTime); %add cp such that the data is cut at 30ms
...
if concatenate_phases
    ...
    for isess = 1:length(phase_paths)
        ...
        cd(fullfile(phase_paths{isess}, '100iter_cut@55/phase'));
        ...
        cd(fullfile(phase_paths{isess}, '100iter_cut@55/phase', num2str(chan_nums(1))));
        load('phase.mat'); 
        ...
        for idx = 1:length(active_ch)
            ichan = active_ch(idx);
            cd(fullfile(phase_paths{isess}, '100iter_cut@55/phase', num2str(chan_nums(idx))));
            load('phase.mat');
