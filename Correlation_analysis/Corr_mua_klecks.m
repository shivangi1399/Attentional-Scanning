...
output_folder = '/mnt/hpc/projects/MWSampling/4Shivangi/results_klecks/phase_correlation/cp10_till_100';
permut_n = 1000;
...
for i = 1:64
    ...
    cfg{i}.outfile = fullfile(output_folder, 'mua','all_loc_difflev');
    cfg{i}.perm_indices = perm_indices;
end

slurmfun(@circlin_correlation_mua, cfg, ...
    'partition',     '8GB', ...
    'stopOnError',   false, ...
    'useUserPath',   true);
...
for iloc = 1:length(targ_loc)
    ...
    % Launch jobs
    slurmfun(@circlin_corr_select_trl_mua, cfg, ...
        'partition',   '8GB', ...
        'stopOnError', false, ...
        'useUserPath', true);
end
...
cd(output_folder)
load('ph_all_sess.mat')
output_folder_s = fullfile(output_folder,'mua','only_hits');
...
save_root = '/mnt/hpc/projects/MWSampling/4Shivangi/Plots/correlation/klecks/cp10_till_100/mua/all_loc_difflev';
...
save_root = '/mnt/hpc/projects/MWSampling/4Shivangi/Plots/correlation/klecks/cp10_till_100/mua/loc_difflev/all_dlev';
if ~exist(save_root, 'dir')
    mkdir(save_root);
end
...
for iloc = 1:length(targ_loc)
    ...
    for ch = 1:nCh
        ch_folder = fullfile(output_folder, 'mua',...
            'loc_difflev_all', ...
            sprintf('loc%d', loc), ...
            sprintf('%d_%d', min_th, max_th), ...
            num2str(ch));
...
output_folder_s = fullfile(output_folder,'only_hits');
cd(output_folder)
load('ph_all_sess.mat')
% load the max min stuff here depending on what you want to plot
...
% Folder to save all figures
save_root = '/mnt/hpc/projects/MWSampling/4Shivangi/Plots/correlation/klecks/cp10_till_100/lfp/only_hits/loc_difflev/all_dlev';
if ~exist(save_root, 'dir')
    mkdir(save_root);
end
...
for iloc = 1:length(targ_loc)
    ...
    for ch = 1:nCh
        ch_folder = fullfile(output_folder_s,'mua', ...
            'loc_difflev_all', ...
            sprintf('loc%d', loc), ...
            sprintf('%d_%d', min_th, max_th), ...
            num2str(ch));
