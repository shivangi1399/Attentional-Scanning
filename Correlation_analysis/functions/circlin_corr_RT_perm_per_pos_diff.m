function circlin_corr_RT_perm_per_pos_diff(cfg_fun)
% H3 permutation: shuffle RT globally across hit trials, then compute
% circ_corrcl within each (position x difficulty bin) cell, average across
% cells.
...
for perm = 1:permut_n
    ...
    for c = 1:nCell
        ...
        rt_c  = rt_c(valid);
        for foi = 1:nFreq
            corr_cell(c,foi) = circ_corrcl(ph_c(:,foi), rt_c);
        end
    end
