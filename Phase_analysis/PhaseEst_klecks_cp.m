...
if run_iter
    ...
    if zscore_run
        for isess = 1:length(session_names)
            ...
            if ~isdir(fullfile(output_paths{isess},'100iter_cut@cp_m20'))
                mkdir(fullfile(output_paths{isess},'100iter_cut@cp_m20'))
            end
            ...
            for ichan = 1:length(clean_data.label)
                ...
                if ~isempty(idx)
                    cp = CriticalTime(idx)-0.02; % get corresponding critical time
            ...
            cd(fullfile(output_paths{isess},'100iter_cut@cp_m20'))
    ...
    for isess = 1:length(session_names)
        ...
        for iter = 1:iter_n
            ...
            if ~isdir(fullfile(output_paths{isess},'100iter_cut@cp_m20',num2str(iter)))
                mkdir(fullfile(output_paths{isess},'100iter_cut@cp_m20',num2str(iter)))
            end

            cfg{iter}.inputfile   = fullfile(output_paths{isess},'100iter_cut@cp_m20','zdat.mat');
            cfg{iter}.exms        = exms;
            ...
            cfg{iter}.outputfile  = fullfile(output_paths{isess},'100iter_cut@cp_m20',num2str(iter));
...
for isess= 1:length(session_names)
    ...
    for ichan = 1:length(clean_data.label)
        ...
        if ~isdir(fullfile(output_paths{isess},'100iter_cut@cp_m20','phase',num2str(ichan)))
            mkdir(fullfile(output_paths{isess},'100iter_cut@cp_m20','phase',num2str(ichan)))
        end
        
        cfg{ichan}.inputfile   = fullfile(output_paths{isess},'100iter_cut@cp_m20');
        cfg{ichan}.ichan       = ichan;
        cfg{ichan}.iter_n      = 100;
        cfg{ichan}.toi         = 3;
        cfg{ichan}.outputfile  = fullfile(output_paths{isess},'100iter_cut@cp_m20','phase',num2str(ichan));
...
for isess = 1:length(session_names)
    isess
    cd(fullfile(output_paths{isess},'100iter_cut@cp_m20',num2str(iter)))
    load freqpow.mat 
