...
zscore_run   = 0;     % if first_run, z-score session (over session) and save this z-scored 
...
if run_iter
    ...
    if zscore_run
        for isess = 1:length(session_names)
            ...
            if ~isdir(fullfile(output_paths{isess},'100iter_cut@cp'))
                mkdir(fullfile(output_paths{isess},'100iter_cut@cp'))
            end
            ...
            for ichan = 1:length(clean_data.label)
                ...
                if ~isempty(idx)
                    cp = CriticalTime(idx)-0.01; % get corresponding critical time
            ...
            cd(fullfile(output_paths{isess},'100iter_cut@cp'))
    ...
    for isess = 31:length(session_names)
        isess
        cfg = [];
        cfg = cell(1,iter_n);
        ...
        for iter = 1:iter_n
            ...
            if ~isdir(fullfile(output_paths{isess},'100iter_cut@cp',num2str(iter)))
                mkdir(fullfile(output_paths{isess},'100iter_cut@cp',num2str(iter)))
            end

            cfg{iter}.inputfile   = fullfile(output_paths{isess},'100iter_cut@cp','zdat.mat');
            cfg{iter}.exms        = exms;
            ...
            cfg{iter}.outputfile  = fullfile(output_paths{isess},'100iter_cut@cp',num2str(iter));
...
for isess= 1:length(session_names)
    ...
    for ichan = 1:length(clean_data.label)
        ...
        if ~isdir(fullfile(output_paths{isess},'100iter_cut@cp','phase',num2str(ichan)))
            mkdir(fullfile(output_paths{isess},'100iter_cut@cp','phase',num2str(ichan)))
        end
        
        cfg{ichan}.inputfile   = fullfile(output_paths{isess},'100iter_cut@cp');
        cfg{ichan}.ichan       = ichan;
        cfg{ichan}.iter_n      = 100;
        cfg{ichan}.toi         = 3;
        cfg{ichan}.outputfile  = fullfile(output_paths{isess},'100iter_cut@cp','phase',num2str(ichan));
...
for isess = 1:length(session_names)
    isess
    cd(fullfile(output_paths{isess},'100iter_cut@cp',num2str(iter)))
    load freqpow.mat 
