...
if stepA
    for isess = 1:length(sesspaths)
        ...
        try
            ...
            cd(sesspaths{isess});
            ESIload('lfpTrials_clean'); % cleaned lfp data
...
%             
            %% Remove channels/trials in muaTrials based on NaNs in lfpTrials_clean
            nTrials = length(lfpTrials_clean.trial);
            for t = 1:nTrials
                nanCh = any(isnan(lfpTrials_clean.trial{t}), 2); % channels with NaNs in this trial
    ...
    if LFP
        for isess = [4,6,10,16,23] %1:length(sesspaths)
            ...
            try
                ...
                close all
                FigH = figure('Position', get(0, 'Screensize'));
                for ichan = 60:63%length(lfpTrials_clean.label)
                    subplot(2,2,ichan)
    ...
    if LFP
        for isess = [6,23] %1:length(sesspaths)
            ...
            try
                ...
                cd(sesspaths{isess}),
                ESIload(fullfile(sesspaths{isess},'lfpTrials_clean.mat'));
                lfpTrials = lfpTrials_clean;
                cfg = [];
                %cfg.method = 'summary';
                %cfg.keepchannel = 'nan';
                %cfg.keeptrial   = 'nan';
                %cfg.latency     = [-1 0.4];
                lfpTrials_clean = ft_databrowser(cfg,lfpTrials);
