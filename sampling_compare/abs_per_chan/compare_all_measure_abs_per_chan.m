...
for ch = 1:nCh
    ...
    tmp = load(val_path, val_var);
    val = tmp.(val_var);
    tmp = load(perm_path, perm_var);
    prm = tmp.(perm_var);
...
[corr_mua, thr_corr_mua, nc_mua] = load_coh_avg(...
    ...
    'corr_perm.mat', 'corr_perm', nCh, 'all_loc_difflev');
...
[corr_hm, thr_corr_hm, nc_hm] = load_coh_avg(...
    ...
    'pos_perm.mat', 'pos_perm', nCh, 'all_loc_difflev');
...
if has_reg
    ...
    % Find first session folder for freqpow.mat
...
%% COLOR SCHEME — pastel shading + darker line per row

% Row colors: MUA=teal, LFP=lavender/purple, RT=peach/coral, Hit/Miss=sage/mint
line_colors = [
...
thr_colors = [
    0.40 0.40 0.40;   % grey threshold lines (same for all)
    0.40 0.40 0.40;
    0.40 0.40 0.40;
    0.40 0.40 0.40;
];
...
f1 = figure('Name', ['Phase Analysis - ' animal], ...
...
sig_corr_lfp = build_sig_map(fullfile(corr_root,'lfp'), 'correlation.mat','correlation', 'corr_perm.mat','corr_perm', nCh, 'all_loc_difflev', nFreq);
sig_corr_rt  = build_sig_map(fullfile(corr_root,'RT'),  'correlation.mat','correlation', 'corr_perm.mat','corr_perm', nCh, 'all_loc_difflev', nFreq);
sig_corr_hm  = build_sig_map(fullfile(corr_root,'hit_miss'), 'pos.mat','pos', 'pos_perm.mat','pos_perm', nCh, 'all_loc_difflev', nFreq);
...
% Plot heatmaps
f2 = figure('Name', ['Significance Heatmaps - ' animal], ...
    'Units', 'centimeters', 'Position', [1 1 48 38]);
...
for row = 1:4
    ...
    % Custom 2-color colormap: light grey -> row's pastel color
    cmap_row = [linspace(0.92, sc(1), 64)', ...
                linspace(0.92, sc(2), 64)', ...
                linspace(0.92, sc(3), 64)'];

    % Column 1: Coherence
    subplot(4, 3, (row-1)*3 + 1);
    imagesc(freq, 1:nCh, sig_coh_all{row});
    set(gca, 'YDir', 'normal');
    colormap(gca, [0.95 0.95 0.95; lc]);
    caxis([0 1]);
    ...
    if has_reg
        if row <= length(sig_reg_all) && ~isempty(sig_reg_all{row})
...
coh_combined  = fullfile(results_combined, 'phase_coherence',  'abs_per_chan', 'cp10_till_100');
corr_combined = fullfile(results_combined, 'phase_correlation', 'abs_per_chan', 'cp10_till_100');
reg_combined  = fullfile(results_combined, 'multi_lin_reg',     'abs_per_chan', 'cp10_till_100');
