% Compare all phase-dependent analyses (H3: abs per stimulus position x
% difficulty bin). Same layout as compare_all_measures.m / _abs_per_pos.m but
% loads from the H3 result folders. Saved values are already real magnitudes
% (per-cell abs() taken inside the analysis), so no extra abs() wrapping.
...
%% HELPER: load and average across channels (H3 values already magnitudes)
...
[coh_mua, thr_coh_mua, n_coh_mua] = load_coh_avg(...
    ...
    'coh_perm_pos_diff.mat', 'coh_perm_pos_diff', nCh, 'all_loc_difflev');
...
[coh_hm, thr_coh_hm, n_coh_hm] = load_coh_avg(...
    ...
    'itc_perm_pos_diff.mat', 'itc_perm_pos_diff', nCh, 'all_loc_difflev');
...
[corr_mua, thr_corr_mua, nc_mua] = load_coh_avg(...
    ...
    'corr_perm_pos_diff.mat', 'corr_perm_pos_diff', nCh, 'all_loc_difflev');
...
[corr_hm, thr_corr_hm, nc_hm] = load_coh_avg(...
    ...
    'pos_perm_pos_diff.mat', 'pos_perm_pos_diff', nCh, 'all_loc_difflev');
...
f1 = figure('Name', ['Phase Analysis (Abs-Per-Pos-Diff) - ' animal], ...
...
sig_coh_mua = build_sig_map(fullfile(coh_root,'mua'), 'coherence.mat','coh', 'coh_perm_pos_diff.mat','coh_perm_pos_diff', nCh, 'all_loc_difflev', nFreq);
sig_coh_lfp = build_sig_map(fullfile(coh_root,'lfp'), 'coherence.mat','coh', 'coh_perm_pos_diff.mat','coh_perm_pos_diff', nCh, 'all_loc_difflev', nFreq);
sig_coh_rt  = build_sig_map(fullfile(coh_root,'RT'),  'coherence.mat','coh', 'coh_perm_pos_diff.mat','coh_perm_pos_diff', nCh, 'all_loc_difflev', nFreq);
sig_coh_hm  = build_sig_map(fullfile(corr_root,'hit_miss'), 'itc.mat','itc', 'itc_perm_pos_diff.mat','itc_perm_pos_diff', nCh, 'all_loc_difflev', nFreq);

sig_corr_mua = build_sig_map(fullfile(corr_root,'mua'), 'correlation.mat','correlation', 'corr_perm_pos_diff.mat','corr_perm_pos_diff', nCh, 'all_loc_difflev', nFreq);
sig_corr_lfp = build_sig_map(fullfile(corr_root,'lfp'), 'correlation.mat','correlation', 'corr_perm_pos_diff.mat','corr_perm_pos_diff', nCh, 'all_loc_difflev', nFreq);
sig_corr_rt  = build_sig_map(fullfile(corr_root,'RT'),  'correlation.mat','correlation', 'corr_perm_pos_diff.mat','corr_perm_pos_diff', nCh, 'all_loc_difflev', nFreq);
sig_corr_hm  = build_sig_map(fullfile(corr_root,'hit_miss'), 'pos.mat','pos', 'pos_perm_pos_diff.mat','pos_perm_pos_diff', nCh, 'all_loc_difflev', nFreq);
...
f2 = figure('Name', ['Significance Heatmaps (Abs-Per-Pos-Diff) - ' animal], ...
    'Units', 'centimeters', 'Position', [1 1 48 38]);
...
coh_combined  = fullfile(results_combined, 'phase_coherence',   'abs_per_pos_diff', 'cp10_till_100');
corr_combined = fullfile(results_combined, 'phase_correlation', 'abs_per_pos_diff', 'cp10_till_100');
reg_combined  = fullfile(results_combined, 'multi_lin_reg',     'abs_per_pos_diff', 'cp10_till_100');
...
f3 = figure('Name', 'Phase Analysis (Abs-Per-Pos-Diff) - Monkey Average', ...
    'Units', 'centimeters', 'Position', [1 1 48 38]);
