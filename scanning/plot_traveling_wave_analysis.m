...
%
% Tests whether the position-dependent preferred phase across channels
% is consistent with a traveling wave propagating across cortex, and
% characterizes the spatial structure of the phase pattern.
%
% Runs for each (animal x DV) combination. Controlled by CH_FILTER:
%   'all'      -- all 64 channels
%   'sig_only' -- only channels significant in the H2-H1 test
%
% Nine-page PDF per (animal, DV):
%
% --- Spatial structure on the array ---
%
%   Page 1 -- Phase heatmap on the 8x8 grid. Each cell = one
%             channel's preferred phase (HSV colormap, -pi to pi).
%             White squares mark driven channels (RF overlaps stimulus).
%             A smooth color gradient across the grid = spatial
%             organization of phase.
%
%   Page 4 -- Summary: same heatmap for 3 representative positions
%             (peripheral, middle, foveal) with the fitted gradient
%             arrow from Page 3 overlaid in black.
%
%   Page 5 -- Moran's I: spatial autocorrelation of cos(phase) on
%             the grid using queen-contiguity weights (8-connected
%             neighbors). Positive I = neighboring electrodes have
%             similar phases (spatially smooth); I near 0 = random.
%             P-value from 1000 permutations of phase across electrode
%             locations. Tests whether the phase map is more spatially
%             organized than chance, without assuming any wave model.
%
% --- Where and how does phase change with position? ---
%
%   Page 2A -- Phase change map. Shows the circular phase difference
%              (deg) between two positions on the 8x8 grid. Three
%              panels: peripheral vs foveal, peripheral vs middle,
%              middle vs foveal. Bright colors = large phase shift
%              between those positions; near-zero = same phase.
%              Directly visualizes which channels change phase.
%
%   Page 2B -- Circular variance map. Left: for each channel, the
%              circular variance of phase across all positions (0 =
%              identical phase at every position, 1 = uniformly
%              spread). Plotted on the 8x8 grid. High values = phase
%              depends on position (H2-type channel). Right: histogram
%              of circular variance for sig vs non-sig channels. If
%              sig channels have higher variance, that validates H2-H1.
%
%   Page 2C -- Phase trajectory per significant channel. One line per
%              channel, x = stimulus position (peripheral to foveal),
%              y = preferred phase (unwrapped, aligned to 0 at pos 1).
%              Flat lines = phase doesn't change. Diverging lines =
%              positions disagree on phase. Similar trajectories for
%              neighboring channels = spatially coherent effect.
%
%   Page 6 -- Phase vs retinotopic distance (sig channels only).
%             For each channel, distance from RF center to estimated
%             stimulus location. Tests whether phase follows the
%             visual field map.
%
% --- Phase gradient characterization ---
%
%   Page 3 -- 2D plane fit. For each position, fit phase = a*row +
%             b*col + c to the (2D-unwrapped) phases across channels.
%             Left: R2 per position (how well a plane explains the
%             phase pattern). Middle: gradient magnitude sqrt(a^2+b^2)
%             in deg/electrode. Right: gradient direction vectors
%             (a,b) as arrows -- shows how the gradient rotates across
%             positions.
%
% Output:
